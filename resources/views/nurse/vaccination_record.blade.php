<div class="vacc_rec_{{ $id }}">
    ...
    <div class="row vacc_rec_institution">
        ...
        <div class="form-group col-md-12">
            ...
            <select class="form-control mid_spe_status" name="imm_status_status[{{ $id }}][]">
        ...
        <div class="form-group col-md-12" style="display: {{$id == 12 ? 'block' : 'none'}}">
            ...
            <select class="form-control mid_spe_status" name="covid_dose[{{$id}}]"  onchange="handleDoseChange(this, {{ $id }})">
        ...
        <div class="col-md-12" style="display: {{$id == 12 ? 'none' : 'block'}}">
            ...
            <div>
                @foreach ($evidence_types as $evidence)
                    <input type="radio" name="evidence_required[{{ $id }}][]" value="{{ $evidence->id }}" {{$vaccination_data!=''?($vaccination_data->evidance_type==$evidence->id?'checked':''):''}}>
        ...
        <div class="col-md-12" id="evidence_required_container">
            ...
            <div id="evidence_div">
                ...
                if($vaccination_data!='' && $vaccination_data->covid_dose!=0){
                    ...
                    <input type="radio" name="evidence_required[{{ $id }}][]" value="{{ $evidence1->id }}" {{$vaccination_data!=''?($vaccination_data->evidance_type==$evidence1->id?'checked':''):''}}>
                    <label>{{ $evidence->name }}</label><br>
                    <?php }
                }
                ?>
        ...
        $.each(evidenceTypes, function (index, data) {
            if (data.dose == selectedDose) { 
                ...
                evidenceRequiredDiv.append(`
                    <input type="radio" id="evidence_re-${index}-${i}" name="evidence_required[${data.type}][]" value="${data.id}" ${data.id == selectedEvidenceId ? 'checked' : ''}>
