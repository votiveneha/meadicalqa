...
<main class="main find_job_div">
   <section class="section-box mt-30">
      <div class="container">
         ...
         <div class="saved-searches-row" id="search-tabs">
            <div class="saved-search-tab active linked" data-id="1">
               My Preferences
               <div class="unsaved-dot"></div>
            </div>
            <div class="saved-search-tab add-new">+ Save New</div>
         </div>
         <div class="job_tabs">
            <ul class="nav nav-tabs">
               <li class="active"><a data-toggle="tab" href="#find_jobs">Find Jobs</a></li>
               <li><a data-toggle="tab" href="#manged_saved_searches">Manage Saved Searches</a></li>
            </ul>
         </div>
         <div class="tab-content">
            <div id="find_jobs" class="tab-pane fade in active">
            ...
            <div id="manged_saved_searches" class="tab-pane in fade">
                ...
                <table id="savedSearchTable" class="table">
                  ...
                  <tbody>
                    <tr data-id="1">
                      ...
                      <td><span class="alert-pill alert-on">Daily</span></td>
                      <td>Email</td>
                      <td>Today</td>
                      <td class="actions">
   ...
   <div class="modal-overlay" id="save-search-modal" style="display: none;">
      <div class="modal-content">
         ...
         <input type="text" id="search-name" placeholder="Enter a name">
   ...
   <div class="edit_side_drawer">
      ...
      <div class="drawer" id="drawer">
         ...
         <div class="tabs">
            <div class="tab active" data-tab="filters-tab">Filters</div>
            <div class="tab" data-tab="alerts-tab">Alerts</div>
            <div class="tab" data-tab="details-tab">Name & Notes</div>
         </div>
         <div class="tab-content active" id="filters-tab">
            ...
            <input type="number" id="filter-preview" min="0">
         </div>
         <div class="tab-content" id="alerts-tab">
            ...
            <input type="number" id="alert-cap" min="0">
            <label>Quiet Hours</label>
            <input type="time" id="quiet-start"> - <input type="time" id="quiet-end">
         </div>
...
<script>
   ...
   $('.btn-save').on('click', function() {
     ...
     if (editId) {
       ...
       row.find('td:nth-child(2) .alert-pill').text(alert)
         .removeClass('alert-on alert-off')
         .addClass(alert === "Off" ? "alert-off" : "alert-on");
       row.find('td:nth-child(3)').text(delivery);
       $(`.saved-search-tab[data-id="${editId}"]`).text(name);
     } else {
       const newId = Date.now();
       addSearchToUI(newId, name, alert, delivery);
     }
   ...
   $('.drawer .tab').click(function(){
    ...
    $(document).on('click', '.btn-duplicate', function() {
      const row = $(this).closest('tr');
      const name = row.find('td:first').text() + " Copy";
      const alert = row.find('td:nth-child(2)').text().trim();
      const delivery = row.find('td:nth-child(3)').text().trim();
      addSearchToUI(Date.now(), name, alert, delivery);
    });
   
    $(document).on('click', '.btn-delete', function() {
      if (confirm("Delete this saved search?")) {
        const id = $(this).closest('tr').data('id');
        $(`.saved-search-tab[data-id="${id}"]`).remove();
