                    ...
                    <div class="card shadow-sm border-0 p-4 mt-30">
                      ...
                      <form id="work_environment_form" method="POST" onsubmit="return update_work_preferences()">
                        ...
                        <div class="form-group level-drp">
                            ...
                            <select class="form-input mr-10 select-active" name="sector_preferences" onchange="empType(this.value)">
                              <option value="">select</option>
                              @if(!empty($employeement_type_preferences))
                              @foreach($employeement_type_preferences as $emptype_data)
                              <option value="{{ $emptype_data->emp_prefer_id }}" @if(!empty($emptype_data) && $emptype_data->emp_prefer_id == "Public & Government") selected @endif>{{ $emptype_data->emp_type }}</option>
                              @endforeach
                              @endif
                            </select>
        ...
        $.ajax({
            ...
            success: function(data){
                ...
                $(".emp_data").html('\<div class="emptype_main_div emptype_main_div-'+value+'"><div class="emptypediv emptypediv-'+value+' form-group level-drp">\
                    ...
                    <select class="js-example-basic-multiple'+value+' addAll_removeAll_btn emptype_valid-'+value+' emptype_valid-'+value+'" data-list-id="emptype_field-'+value+'" name="emptypelevel[]" multiple></select>\
                    <span id="reqemptype-'+value+'" class="reqError text-danger valley"></span>\
                    </div></div>');
